...
**Status:** Foundation approved — v1.1 (adds Client, PublicationConsent, expanded status workflow, featured flag)
...
> **Changelog v1.1:** Added `Client` model (admin-managed, no login), `PublicationConsent` model (tracks public/private permission + timestamp), expanded `JourneyStatus` to a full editorial pipeline (`DRAFT → REVIEW → APPROVED → PUBLISHED`, plus `ARCHIVED`), and added `Journey.featured` for homepage curation. Everything else from v1.0 is unchanged.
...
- `Client` is intentionally a plain contact record — no auth fields, no password, nothing login-related. It exists purely so the admin can track who a journey belongs to and reuse client info across repeat bookings.
