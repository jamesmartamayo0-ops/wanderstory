...
export async function migrateRelease(env: Environment, checks: ReleaseChecks = defaults): Promise<void> {
  ...
  const before = compareMigrationHistory(files, await checks.inspect(target, "release-precheck"));
  ...
  const rechecked = compareMigrationHistory(files, await checks.inspect(target, "release-precheck"));
  ...
  const after = compareMigrationHistory(files, await checks.inspect(target, "release-postcheck"));
